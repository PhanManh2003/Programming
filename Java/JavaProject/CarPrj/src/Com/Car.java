...
public class Car implements Comparable<Car> {
    ...
    public int compareTo(Car c) {
        // compareTo ở dưới này là phương thức của lớp String
        int val = this.getBrand().getBrandName().compareTo(c.getBrand().getBrandName());
        if (val == 0) {
            val = this.getCarID().compareTo(c.getCarID());
        }
        return val;
    }
